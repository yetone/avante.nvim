...
function M.setup_test_env(opts)
  ...
  table.insert(ctx.cleanup_funcs, function()
    ...
    if ctx.config_backup then avante_config.options = ctx.config_backup end
...
function M.assert_provider_response(response, provider, expected)
  ...
  if provider == "openai" then
    if expected.type == "chat" then
      ...
      if expected.content then assert.equals(expected.content, response.choices[1].message.content) end
    end
  elseif provider == "claude" then
    if expected.type == "chat" then
      ...
      if expected.content then assert.equals(expected.content, response.content[1].text) end
    end
  end
end
...
function M.stop_mock_server(server) server.running = false end
