  ...
  describe("Chat Completions", function()
    it("should handle successful chat completion", function()
      ...
      if code_opts.on_complete then
        code_opts.on_complete("Hello! I'm doing well, thank you for asking.")
      end
    ...
    it("should handle authentication errors", function()
      ...
      if code_opts.on_error then
        code_opts.on_error("Incorrect API key provided: invalid-key")
      end
    ...
    it("should handle rate limiting errors", function()
      ...
      if code_opts.on_error then
        code_opts.on_error("Rate limit exceeded", { ["Retry-After"] = "60" })
      end
    ...
    it("should handle model not found errors", function()
      ...
      if code_opts.on_error then
        code_opts.on_error("The model 'invalid-model' does not exist")
      end
    ...
    it("should handle network timeouts", function()
      ...
      if code_opts.on_error then
        code_opts.on_error("Connection timeout after 1000ms")
      end
      ...
      for _, chunk in ipairs(test_chunks) do
        if chunk:match("^data: ") and not chunk:match("%[DONE%]") then
          ...
          if success and data.choices and data.choices[1] and data.choices[1].delta then
            if data.choices[1].delta.content then
              parsed_content = parsed_content .. data.choices[1].delta.content
            end
