...
import { DiscordStats } from '@/lib/types';

type Data = DiscordStats | { error: string };

/**
 * API endpoint to fetch Discord community statistics
...
export default async function handler(
  ...
  try {
    ...
    if (!stats) {
      return res.status(500).json({ error: 'Failed to fetch Discord stats' } as any);
    }
    ...
    console.error('Error in Discord stats API:', error);
