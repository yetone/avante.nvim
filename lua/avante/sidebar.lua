...
local DEBOUNCE_MS = 50 -- 50ms debounce delay
  ...
  local function handle_submit(request)
    ...
    local on_chunk = function(chunk)
      ...
      if is_first_chunk then
        ...
        vim.schedule(function() self:update_content(content_prefix .. chunk, { scroll = scroll }) end)
        return
      end
      local suffix = get_display_content_suffix(transformed)
      vim.schedule(function() self:update_content(content_prefix .. cur_displayed_response .. suffix, { scroll = scroll }) end)
