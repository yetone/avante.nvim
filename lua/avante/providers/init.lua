...
function E.setup(opts)
  ...
  local function mount_input_buffer()
    vim.defer_fn(function()
      ...
      if not vim.tbl_contains(exclude_filetypes, vim.bo.filetype) and not opts.provider.is_env_set() then
        InputWindowState.winid = api.nvim_get_current_win()
        vim.ui.input({ default = "", prompt = "Enter " .. var .. ": " }, on_confirm)
        for _, winid in ipairs(api.nvim_list_wins()) do
          local bufnr = api.nvim_win_get_buf(winid)
          if vim.bo[bufnr].filetype == InputWindowConfig.filetype then
            InputWindowState.input_winid = winid
            InputWindowState.input_bufnr = bufnr
        ...
        local prompt_length = api.nvim_strwidth(fn.prompt_getprompt(InputWindowState.input_bufnr))
        api.nvim_buf_call(
          InputWindowState.input_bufnr,
          function()
  ...
  if refresh then return mount_input_buffer() end

  api.nvim_create_autocmd("User", {
    pattern = E.REQUEST_LOGIN_PATTERN,
    callback = mount_input_buffer,
  })
end
