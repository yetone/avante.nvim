...
function M.read_definitions(bufnr, symbol_name, show_line_numbers, on_complete)
  ...
  vim.lsp.buf_request(bufnr, "workspace/symbol", params, function(err, result)
    if err then
      on_complete(nil, err.message)
      return
    end
    local definitions = vim.tbl_filter(function(d) return d.name == symbol_name end, result)
    ...
    on_complete(res, nil)
  end, function() on_complete(nil, "unsuported filetype") end)
end
