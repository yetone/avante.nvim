...
function M.log_prompt(request)
  local log_dir = Config.prompt_logger and Config.prompt_logger.log_dir
      or vim.fn.expand('~/.cache/nvim/avante_logs')
...
local function refresh_logs()
  local dir = Config.prompt_logger and Config.prompt_logger.log_dir
      or vim.fn.expand('~/.cache/nvim/avante_logs')
  logs = vim.fn.glob(dir .. '/avante_prompt_*.log', false, true)
...
local function load_log(step)
  ...
  if #logs == 0 then
    vim.notify('No prompt logs found 🤷', vim.log.levels.WARN); return
  end
  ...
  if not fp then
    vim.notify('Could not open ' .. logs[idx], vim.log.levels.ERROR); return
  end
