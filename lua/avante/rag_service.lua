...
function M.launch_rag_service(cb)
        ...
        if M.get_rag_service_runner() == "docker" then
                ...
                if result ~= "" then
                        Utils.info(string.format("container %s already started but not running, stopping...",
                                container_name))
                        M.stop_rag_service()
                end
                ...
                vim.fn.jobstart(cmd_, {
                        ...
                        on_exit = function(_, exit_code)
                                if exit_code ~= 0 then
                                        Utils.error(string.format("container %s failed to start, exit code: %d",
        ...
        elseif M.get_rag_service_runner() == "nix" then
                ...
                local dirname =
                    Utils.trim(string.sub(debug.getinfo(1).source, 2, #"/lua/avante/rag_service.lua" * -1),
                            { suffix = "/" })
                ...
                vim.fn.jobstart(cmd, {
                        ...
                        on_exit = function(_, exit_code)
                                if exit_code ~= 0 then
                                        Utils.error(string.format("service %s failed to start, exit code: %d",
                                                container_name, exit_code))
                                else
...
function M.add_resource(uri)
        ...
        print(vim.inspect(resources_resp))
