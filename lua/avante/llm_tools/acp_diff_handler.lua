...
function M.extract_diff_blocks(tool_call)
  ...
  for _, content_item in ipairs(tool_call.content) do
    if content_item.type == "diff" then
      ...
      --FIXIT: Remove debug dd statements later
      if oldText == vim.NIL or oldText == nil then
        dd("oldText", {
          value = content_item.oldText,
          type = type(content_item.oldText),
          is_nil = oldText == nil,
          is_vim_nil = oldText == vim.NIL,
        })
      end
  ...
  -- Only process if no diff blocks were found in content
  if #diff_blocks_by_file == 0 and tool_call.rawInput then
    local raw = tool_call.rawInput
    if raw then
      ...
      local new_string = raw.new_string
