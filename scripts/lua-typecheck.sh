...
handle_managed_mode() {
    ...
    log "Setting up managed environment in: $dest_dir"
    mkdir -p "$dest_dir"
    ...
    log_verbose "Installing lua-language-server via setup script..."
...
main() {
    ...
    local dest_dir="$PWD/target/typecheck"
    local luarc_path="$dest_dir/luarc.json"

    if [ "$mode" == "live" ]; then
        handle_live_mode
    elif [ "$mode" == "managed" ]; then
        handle_managed_mode "$dest_dir" "$setup_deps_flags"
        manage_plugin_dependencies "$dest_dir/deps" "$setup_deps_flags"
    elif [ "$mode" == "ci" ]; then
        # In CI, nvim is installed by the workflow.
        # We need to install luals and plugin dependencies.
        local luals_bin_path
        luals_bin_path="$(./scripts/setup-deps.sh "$setup_deps_flags" install-luals "$PWD/luals")"
        export PATH="$luals_bin_path:$PATH"
        log_verbose "Added $luals_bin_path to PATH for CI"

        local deps_dir="${GITHUB_WORKSPACE:-$PWD}/deps"
        manage_plugin_dependencies "$deps_dir" "$setup_deps_flags"

        luarc_path="${GITHUB_WORKSPACE:-$PWD}/luarc.json"
    fi
